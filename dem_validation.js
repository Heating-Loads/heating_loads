...
var glo_elev = glo30.mosaic().setDefaultProjection('EPSG:3857',null,30)

//proposal
//take min elevation in bldg footprint-- FABDEM 

//AK buildings (Erin's OSM table)
var bldgs = ee.FeatureCollection('users/edtrochim/OSM_building_AK').filter(ee.Filter.bounds(geometry));
...
var fabdem_elev = fabdem.mosaic().setDefaultProjection('EPSG:3857',null,30)
//var elev = fabdem.mosaic().setDefaultProjection(glo30.first().projection())

var min_elev = fabdem_elev.reduceRegions({collection: bldgs,
                  reducer: ee.Reducer.min(),
                  scale:10
})

var min_elev_glo = glo_elev.reduceRegions({collection: bldgs,
                  reducer: ee.Reducer.min(),
                  scale:10
})

var max_elev = fabdem_elev.reduceRegions({collection: bldgs,
                  reducer: ee.Reducer.max(),
                  scale:10
})
var max_elev_glo = glo_elev.reduceRegions({collection: bldgs,
                  reducer: ee.Reducer.max(),
                  scale:10
})
